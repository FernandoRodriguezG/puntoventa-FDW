...
<head>
  ...
  <title>Busca cliente | Sistema de Inventario</title>
...
<div class="container-fluid">
    ...
    <form action="buscaCliente.php" method="get" onsubmit="return validaBusqueda()">
        ...
        if(isset($_GET['criterio']) && isset($_GET['txtCriterio'])){
            if($_GET['criterio']!=0 && $_GET['txtCriterio']!=""){
                if($_GET['criterio']=="Nombre"){
                    $consulta = "select * from clientes where Nombre like '%".$_GET['txtCriterio']."%'";
                }else{
                    $consulta = "select * from clientes where Correo like '%".$_GET['txtCriterio']."%'";
                }
